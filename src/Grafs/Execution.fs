...
open Grafs.Ast
open Grafs.Types
open Grafs.Types.Patterns
open Grafs.Planning

type Error = string * string list

type GQLValue =
    | GQLObj of struct (string * GQLValue) list
    | GQLList of GQLValue list
    | GQLNull
    | GQLInt of int
    | GQLFloat of double
    | GQLBool of bool
...
module Json =
    ...
    let rec toJson (writer: TextWriter) = 
        function
        | GQLNull     -> writer.Write("null")
        | GQLInt n    -> writer.Write(n)
        | GQLFloat n  -> writer.Write(n)
        | GQLBool b   -> writer.Write(if b then "true" else "false")
        | GQLString s -> 
            writer.Write('"')
            writer.Write(s)
            writer.Write('"')
        | GQLList l   -> 
            let rec write' (writer: TextWriter)=
                function
                | [] -> ()
                | [h] -> toJson writer h
                | h::t  -> 
    ...
    type GQLValue with
        member x.ToJson (writer: TextWriter) = toJson writer x
        member x.ToJson () =
...
module internal Execution =
    ...
    let rec private buildResolverTree (returnDef: OutputDef) (ctx: ResolveFieldContext) (fieldExecuteMap: FieldExecuteMap) (value: obj option) : AsyncVal<ResolverTree> =
        ...
        | List (Output innerdef) ->
            ...
            | SomeObj(:? System.Collections.IEnumerable as enumerable) ->
                ...
                |> Seq.toList
                |> build [] 
            | _ -> raise <| GQLException (sprintf "Expected to have enumerable value in field '%s' but got '%O'" ctx.ExecutionInfo.Identifier (value.GetType()))
        ...
        | Interface idef ->
            ...
            | Some v ->
                ...
                | None -> asyncVal { return ResolverError { Name = name; Message = ctx.Schema.ParseError (GQLException (sprintf "GraphQL Interface '%s' is not implemented by the type '%s'" idef.Name resolvedDef.Name)); PathToOrigin = [] } }
            | None -> 
                if ctx.ExecutionInfo.IsNullable 
                then asyncVal { return ResolverObjectNode { Name = name; Value = None; Children = [| |] } } 
                else nullResolverError name
            ...
            | Some v ->
                ...
                | Some fields -> 
                    ...
                    |> AsyncVal.map(fun tree -> 
                        ...
                        | t -> t)
                | None -> asyncVal { return ResolverError { Name = name; Message = ctx.Schema.ParseError (GQLException (sprintf "GraphQL Union '%s' is not implemented by the type '%s'" udef.Name resolvedDef.Name)); PathToOrigin = [] } }
            | None -> 
                if ctx.ExecutionInfo.IsNullable 
                then asyncVal { return ResolverObjectNode { Name = name; Value = None; Children = [| |] } } 
                else nullResolverError name
        ...
        let rec build (acc: ResolverTree list) = function
            | info::xs ->
                ...
                |> AsyncVal.bind(fun tree ->
                    ...
                    | t when not info.IsNullable && tree.Value.IsNone -> asyncVal { return ResolverError { Name = name; Message = ctx.Schema.ParseError(GQLException (sprintf "Non-Null field %s resolved as a null!" info.Identifier)); PathToOrigin = [info.Identifier]}} 
                    | t -> build (t::acc) xs)
    ...
    let internal compileSubscriptionField (subfield: SubscriptionFieldDef) = 
        ...
        | _ -> raise <| GQLException ("Invalid filter expression for subscription field!")
                                ...
                                let! res = 
                                    ...
                                    | p, ResolverObjectNode n -> 
                                        asyncVal {
                                            ...
                                            let! res =
                                                match next with
                                                | Some next' -> traversePath p (AsyncVal.ofValue next') ((List.head p)::pathAcc)
                                                | None -> AsyncVal.empty
                                            return res
                                        }
                                    | p, ResolverListNode l ->
                                        asyncVal {
                                            let! res =
                                                l.Children
                                                |> Array.mapi(fun i c -> 
                                                    traversePath p (AsyncVal.ofValue c) (i.ToString()::pathAcc))
                                                |> AsyncVal.collectParallel
                                            return res |> Array.fold (Array.append) [||]
                                        }
                                    | _ ,_ -> raise <| GQLException("Deferred path terminated unexpectedly!")
                                return res
                            }

                        traversePath path (AsyncVal.ofValue tree) [(List.head path)]
    ...
    let internal evaluate (schema: #ISchema) (executionPlan: ExecutionPlan) (variables: Map<string, obj>) (root: obj) errors (fieldExecuteMap: FieldExecuteMap) : AsyncVal<GQLResponse> =
        ...
        | Subscription ->
            ...
            | Some s -> 
                AsyncVal.ofValue(Stream(executeSubscription resultSet ctx s fieldExecuteMap schema.SubscriptionProvider root))
