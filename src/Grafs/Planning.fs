...
module internal Grafs.Planning
...
open Grafs.Types.Introspection
...
let SchemaMetaFieldDef = 
    Define.Field(
        ...
        typedef = Introspection.__Schema,
    ...
    | Some fdef ->
        { Identifier = field.AliasOrName
          ...
          ReturnDef = 
            ...
            | _ -> raise (GQLException (sprintf "Unexpected parentdef type!"))
          Definition = fdef
          Ast = field
          Include = includer 
          IsNullable = false }
    | None ->
        raise (GQLException (sprintf "No field '%s' was defined in object definition '%s'" field.Name parentDef.Name))
    ...
    | Some typeName ->
        ...
        | None -> 
            ...
            | _ ->
                ...
                raise (GQLException (sprintf "There is no object type named '%s' that is a possible type of '%s'" typeName pname.Name))
...
let private directiveIncluder (directive: Directive) : Includer =
    fun variables ->
        ...
        | other -> 
            ...
            | Some s -> s
            | None -> raise (GQLException (sprintf "Expected 'if' argument of directive '@%s' to have boolean value but got %A" directive.Name other))
    ...
    | Mutation ->
        ...
        | Some mutationDef ->
            { DocumentId = documentId
              ...
              Variables = variables }
        | None -> 
            raise (GQLException "Tried to execute a GraphQL mutation on schema with no mutation type defined")
    | Subscription ->
        ...
        | Some subscriptionDef ->
            { DocumentId = documentId
              ...
              Variables = variables }
        | None -> 
            raise (GQLException "Tried to execute a GraphQL subscription on schema with no mutation type defined")