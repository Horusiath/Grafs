...
let rec internal compileByType (errMsg: string) (inputDef: InputDef): ExecuteInput =
    ...
    | InputObject objdef -> 
        ...
        let ctor = Reflection.matchConstructor objtype (objdef.Fields |> Array.map (fun x -> x.Name))
        let mapper =
            ctor.GetParameters()
            |> Array.map(fun param -> 
    ...
    | Nullable (Input innerdef) -> 
        ...
        fun variables value ->
            ...
            | coerced -> 
                let c = some coerced
                if c <> null then c
                else raise(GQLException (errMsg + notAssignableMsg innerdef coerced))
    | Enum enumdef -> 
        fun value variables ->
            match value with
            | Variable variableName -> variables.[variableName]
...
let rec private coerceVariableValue isNullable typedef (vardef: VarDef) (input: obj) (errMsg: string) : obj = 
    ...
    | List (Input innerdef) ->
        ...
        | :? System.Collections.IEnumerable as iter -> 
            ...
            mapped
        | other -> raise (GQLException <| errMsg + (sprintf "Cannot coerce value of type '%O' to list." (other.GetType())))
    | InputObject objdef -> coerceVariableInputObject objdef vardef input (errMsg + (sprintf "in input object '%s': " objdef.Name))
    | _ -> raise (GQLException <| errMsg + "Only Scalars, Nullables, Lists and InputObjects are valid type definitions.")
...
let internal coerceVariable (vardef: VarDef) (inputs) =
    ...
    | None -> 
        ...
        | None -> 
            ...
            | Nullable _ -> null
            | _ -> raise (GQLException (sprintf "Variable '$%s' of required type %s has no value provided." vname (vardef.TypeDef.ToString())))
