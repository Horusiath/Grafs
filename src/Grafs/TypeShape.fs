...
module internal Grafs.TypeShape
...
type IShapeMethod =
    /// Denotes whether constructor is public
    abstract IsPublic : bool
    /// Denotes the arity of the method arguments
    abstract Arity : int
    /// ConstructorInfo instance
    abstract MethodInfo : MethodInfo
    // A tuple type encoding all arguments passed to the method
    abstract Arguments : TypeShape
    
/// Identifies a constructor implementation shape
and IShapeMethod<'DeclaringType> =
    inherit IShapeMethod
...
and ShapePoco<'Poco> private () =
    ...
    interface IShapePoco with
        ...
        member __.Properties = properties |> Array.map (fun p -> p :> _)
        member __.Methods = methods |> Array.map (fun m -> m :> _)
        member __.IsStruct = isStruct
        member __.Accept v = v.Visit __
